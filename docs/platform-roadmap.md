...
This target starts in M2 as a dev-board firmware prototype. M2.0 selects the `stm32h753-nucleo` target path and adds documentation/build skeleton only. M2.1 adds a host-native compile-only embedded skeleton. M2.2 adds host-native platform tick, watchdog, reset-cause, diagnostics, and GPIO/PTT test stubs. It does not include PCB work, RF transmit, real radio keying, USB CDC implementation, real codec drivers, or GPIO PTT drivers.
