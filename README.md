...
Current stage: M1.18 daemon audio backend abstraction with raw-file backend.
...
These platform targets remain planning items except for the minimal host daemon file adapter, raw PCM audio backend abstraction, stdin/stdout mode, localhost-only TCP KISS test adapter, local Unix socket once-mode adapter, and local PTY once-mode adapter.
